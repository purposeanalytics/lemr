...
mod_data_story_agi_tdf_ui <- function(id) {
  map_legend <- shiny::tagList(
    shiny::div(class = "map-legend",
    create_circle_legend(layer_colours[["agi_apartment"]], "Apartment buildings with AGIs", alt_text = "A legend showing the colour of the points of above guideline increase applications for apartment buildings."),
    ...
    shiny::modalDialog(
      ...
      shiny::fluidRow(
        shiny::column(
          ...
          shiny::h1("Above Guideline Increase Applications and Tenant Defence Fund Grants in Toronto: A closer look at three neighbourhoods")
        )
      ),
      shiny::p("A notice of rent increase, recalculating the monthly budget, cutting down on non-essential expenses - or finding a new home. The cycle is not unfamiliar to renters in Toronto. In Ontario, renters are protected by a provincially mandated rent increase guideline that is tied to the Consumer Price Index."),
      shiny::p("However, when landlords incur capital expenses for completing major work, building security, or accessibility features, or extraordinary increases to their property tax, they may apply to pass those costs on to tenants through an Above Guideline Increase, or AGI. In turn, tenants may choose to contest an AGI through the Landlord Tenant Board, a process that can be time-consuming and costly. To support this, the City of Toronto offers assistance, financial and otherwise, through the Tenant Defence Fund (TDF) program to tenants in private rental housing."),
      ...
      shiny::p("Looking at the scatterplot below, we can see that some neighbourhoods had more than twice the rate of AGI applications in apartment buildings than others (neighbourhoods with higher AGI rates are to the right of the chart)."),
      ...
      shiny::div(
          ...
          shiny::p(shiny::HTML("Broadview North, Yonge-St.Clair, and Mimico are highlighted as outliers in the city: the rate of Above Guideline Increase applications or Tenant Defence Fund grants in these neighbourhoods fall outside the norm."))
      ),
      shiny::p("The chart also shows a lot of variation in the percentage of buildings that received Tenant Defence Fund grants. Given that there is no clear relationship here, there are probably other factors at play. To build a better understanding about the relationship between AGIs and TDF grants, we examine three neighbourhoods more closely: Broadview North, Mimico, and Yonge-St. Clair."),
      shiny::fluidRow(
      ...
      shiny::fluidRow(
        ...
        shiny::column(
          width = 8, shiny::h2("Yonge-St. Clair"),
          shiny::p(
      ...
      shiny::fluidRow(
        shiny::column(
          width = 12,
          shiny::p(class="light", shiny::HTML("
For a full data summary, including sociodemographic and housing characteristics either by neighbourhood or city-wide, access the <b>Interactive Map</b>. For data sources and key terms, visit <b>Data & Definitions</b>."))
        )
      ),
      shiny::fluidRow(
        shiny::column(
