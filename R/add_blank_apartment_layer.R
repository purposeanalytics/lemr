...
add_blank_apartment_layer <- function(map) {
  map %>%
    # Add the layer
    mapboxer::add_circle_layer(source = mapboxer::as_mapbox_source(lemur::apartment_building_registry %>% dplyr::select(.data$site_address, .data$geometry, .data$confirmed_units)), id = "apartment_buildings", circle_color = main_colour, circle_blur = 0.5) %>%
