...
add_blank_amenity_density_layer <- function(map) {
  ...
  map %>%
    ...
    mapboxer::add_fill_layer(source = mapboxer::as_mapbox_source(amenity_density_with_colours), fill_color = c("get", "colour"), id = "amenity_density_fill") %>%
    mapboxer::add_line_layer(source = mapboxer::as_mapbox_source(amenity_density_with_colours), line_color = "black", line_width = 0.25, id = "amenity_density_outline") %>%
