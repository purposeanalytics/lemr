...
shiny::tagList(
  purrr::pmap(
    ...
    function(color, wording, data_suffix) {
      lemur:::create_circle_legend(lemur:::rental_supply_colors()[[color]],
  ...
  purrr::pmap(
    ...
    function(color, wording) {
      lemur:::create_circle_legend(lemur:::amenity_density_colours()[[color]],
  ...
  lemur:::create_circle_legend(lemur:::layer_colours[["agi_apartment"]], "Apartment buildings with AGI applications", alt_text = "A legend showing the colour of the points of above guideline increase applications for apartment buildings."),
  lemur:::create_circle_legend(lemur:::layer_colours[["agi_other"]], "Other buildings with AGI applications", alt_text = "A legend showing the colour of the points of above guideline increase applications for apartment buildings."),
  lemur:::create_circle_legend(lemur:::layer_colours[["tdf"]], "TDF grants", alt_text = "A legend showing the colour of the points of tenant defense fund grants.")
)
```
...
lemur:::apartment_building_evaluation_none(apartment_building_evaluation_formatted) %>%
  p() %>%
  lemur:::padded()
...
```{r vacancy-rate, fig.alt = lemur:::vacancy_rate_plot_alt_text(level, neighbourhood)}
vacancy_rate <- lemur:::get_measure(dataset, "vacancy_rate_2020")
vacancy_rate_formatted <- lemur:::format_measure(vacancy_rate, "vacancy_rate")

glue::glue("Primary market vacancy rate: {vacancy_rate_formatted}") %>%
  h3()

lemur:::evictions_description(level, neighbourhood, vacancy_rate, vacancy_rate_formatted) %>%
  p()

lemur:::vacancy_rate_plot(dataset, compare, static = TRUE)
```

### Proximity to services

```{r amenity-density, fig.alt = lemur:::amenity_density_plot_alt_text(level, neighbourhood), fig.height = 2}
...
```{r structure-type, fig.alt = lemur:::structure_type_plot_alt_text(level, neighbourhood), fig.height = 2.5}
...
lemur:::shelter_cost_number(shelter_cost_formatted, level) %>%
  h3()
...
lemur:::unaffordable_housing_number(unaffordable_housing_formatted, level) %>%
  h3()
...
```{r household-tenure, fig.alt = lemur:::household_tenure_plot_alt_text(level, neighbourhood), fig.height = 1.75}
...
```{r bedrooms, fig.alt = lemur:::bedrooms_plot_alt_text(level, neighbourhood), fig.height = 2.5}
...
```{r average-total-household-income, fig.alt = lemur:::average_total_household_income_plot_alt_text(level, neighbourhood), fig.height = 1.75}
...
lemur:::lim_at_number(lim_at_formatted, level) %>%
  h3()
...
```{r household-size, fig.alt = lemur:::household_size_plot_alt_text(level, neighbourhood), fig.height = 2.5}
...
lemur:::visible_minority_number(dataset, level) %>%
  h3()
