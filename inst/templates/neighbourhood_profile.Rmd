...
tm_shape(basemap) +
  ...
  # Rooming houses
  add_circle_layer(dplyr::filter(map_buildings, rooming_house, rooming_house_status %in% c("Licensed prior to 2018", "Licensed 2018 onwards")), lemr:::amenity_density_colours()[["Low"]], level = level) %>%
  add_circle_layer(dplyr::filter(map_buildings, rooming_house, rooming_house_status == "Lapsed"), lemr:::amenity_density_colours()[["High"]], level = level) %>%
  # AGI
  add_circle_layer(dplyr::filter(map_buildings, agi), lemr:::layer_colours[["agi_apartment"]], level = level) %>%
  # TDF
  add_circle_layer(dplyr::filter(map_buildings, tdf), lemr:::layer_colours[["tdf"]], level = level)
# tm_add_legend(type = "symbol", labels = c("Apartment buildings", "Above Guideline Increase applications", "Tenant Defense Fund grants"), col = lemr:::layer_colours, shape = 16) +
...
shiny::tagList(
  ...
  purrr::pmap(
    dplyr::tibble(
      color = c("Low", "High"),
      wording = c("Licensed", "Lapsed")
    ),
    function(color, wording) {
      lemr:::create_circle_legend(lemr:::amenity_density_colours()[[color]],
        glue::glue("{wording} rooming houses"),
        alt_text = glue::glue("A legend showing the colour of the points of {tolower(wording)} rooming houses.")
      )
    }
  ),
  lemr:::create_circle_legend(lemr:::layer_colours[["agi_apartment"]], "Apartment buildings with AGI applications", alt_text = "A legend showing the colour of the points of above guideline increase applications for apartment buildings."),
  lemr:::create_circle_legend(lemr:::layer_colours[["agi_other"]], "Other buildings with AGI applications", alt_text = "A legend showing the colour of the points of above guideline increase applications for apartment buildings."),
  lemr:::create_circle_legend(lemr:::layer_colours[["tdf"]], "TDF grants", alt_text = "A legend showing the colour of the points of tenant defense fund grants.")
)
```
...
lemr:::rental_supply_primary_table(dataset)
```
  </div>
  <div class = "col-sm-6">
```{r rental-supply-secondary-non-market}
lemr:::rental_supply_secondary_table(dataset)
lemr:::rental_supply_non_market_table(dataset)
```
  </div>
  </div>
  </div>
...
```{r apartment-building-units, fig.alt = lemr:::number_of_units_plot_alt_text(level, neighbourhood)}
...
```{r apartment-building-evaluation, fig.alt = lemr:::apartment_building_evaluation_plot_alt_text(level, neighbourhood)}
...
```{r core-housing-need, fig.alt = lemr:::core_housing_need_plot_alt_text(level, neighbourhood)}
...
```{r evictions, fig.alt = lemr:::evictions_plot_alt_text(level, neighbourhood)}
...
```{r vacancy-rate, fig.alt = lemr:::vacancy_rate_plot_alt_text(level, neighbourhood)}
...
glue::glue("Primary market vacancy rate: {vacancy_rate_formatted}") %>%
  h3()
...
```{r apartment-buildings, fig.alt = lemr:::number_of_apartments_plot_alt_text(level, neighbourhood)}
...
```{r shelter-cost, fig.alt = lemr:::average_renter_shelter_cost_plot_alt_text(level, neighbourhood)}
...
```{r unaffordable-housing, fig.alt = lemr:::unaffordable_housing_plot_alt_text(level, neighbourhood)}
...
```{r population-density, fig.alt = lemr:::population_density_plot_alt_text(level, neighbourhood)}
...
```{r population-change, fig.alt = lemr:::population_change_plot_alt_text(level, neighbourhood)}
...
```{r lim-at, fig.alt = lemr:::lim_at_plot_alt_text(level, neighbourhood)}
