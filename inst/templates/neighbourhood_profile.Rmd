...
map_alt_text <- switch(level,
  neighbourhood = glue::glue("A map of {neighbourhood} showing the locations of apartment buildings, evictions hearings, above guideline increase applications, and tenant defense fund grant recipients."),
...
tm_shape(basemap) +
  ...
  add_circle_layer(dplyr::filter(map_buildings, apartment), lemur:::layer_colours[["apartment_buildings"]], level = level) %>%
  add_circle_layer(dplyr::filter(map_buildings, eviction_hearing), lemur:::layer_colours[["evictions_hearings"]], level = level) %>%
  add_circle_layer(dplyr::filter(map_buildings, agi), lemur:::layer_colours[["agi"]], level = level) %>%
  add_circle_layer(dplyr::filter(map_buildings, tdf), lemur:::layer_colours[["tdf"]], level = level) +
  tm_add_legend(type = "symbol", labels = c("Apartment buildings", "Evictions hearings", "Above Guideline Increase applications", "Tenant Defense Fund grants"), col = lemur:::layer_colours, shape = 16) +
...
```{r amenity-density, fig.alt = lemur:::amenity_density_plot_alt_text(level, neighbourhood)}
lemur:::amenity_density_plot(dataset, compare, static = TRUE)

lemur:::generate_table(dataset, "amenity_density", compare, "Amenity density", "Percent") %>%
