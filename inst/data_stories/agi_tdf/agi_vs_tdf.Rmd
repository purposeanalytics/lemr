...
p <- ggplot(buildings_agi_tdf, aes(agi_ratio, tdf_ratio)) +
  ...
  scale_y_continuous(labels = scales::percent, limits = c(0, 1), name = "% of buildings with Tenant Defence Fund grant") +
...
p <- ggplot(buildings_agi_tdf, aes(agi_ratio, tdf_ratio)) +
  ...
  scale_y_continuous(labels = scales::percent, limits = c(0, 1), name = "% of buildings with Tenant Defence Fund grant") +
  scale_x_continuous(labels = scales::percent, limits = c(0, 1), name = "% of buildings with Above Guideline Increase application") +
  labs(title = "Percent of buildings that received an Above Guideline\nIncrease application in the last 5 years versus percent\nof those buildings that received a Tenant Defence Fund\ngrant, by neighbourhood") +
  theme_minimal(base_family = "Lato", base_size = 12) +
  theme(
    legend.position = "none",
    plot.title.position = "plot"
