...
rooming_houses_sf <- readRDS(here::here("data-raw", "points_layers", "rooming_houses", "geocoded", "rooming_houses_sf.rds"))

# remove spati


# look at months that license was active and explore if there are large gaps
rooming_houses_continuity <- rooming_houses_sf %>%
  ...
  ungroup() %>%
  select(address, months_licensed, last_month_licensed, first_month_licensed) %>%
  distinct() %>%
...
rooming_houses_cleaned <- rooming_houses_continuity %>%
  ...
  ungroup() %>%
  select(address, first_month_licensed, last_month_licensed, status) %>%
  distinct()

saveRDS(rooming_houses_cleaned, here::here("data-raw", "points_layers", "rooming_houses", "cleaned", "rooming_housese_cleaned.rds"))
