test_that("clean_neighbourhood_names warns if not all neighbourhoods could be cleaned, and returns the mismatches, plus the full values", {
  expect_warning(dplyr::tibble(neighbourhood = "Donlands (66)") %>% dplyr::mutate(neighbourhood = clean_neighbourhood_names(neighbourhood)), "Donlands \\(66\\)")

  mismatch <- suppressWarnings(dplyr::tibble(neighbourhood = "Donlands (66)") %>% dplyr::mutate(neighbourhood = clean_neighbourhood_names(neighbourhood)))
