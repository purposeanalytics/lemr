...
```{r}
test <- proximity_layers %>% select(neighbourhood, High, Medium, Low,number_of_buildings,`Total_Total`) %>% 
  pivot_longer(cols = c(High, Medium, Low))
```


```{r}
test <- test %>% filter(Total_Total > 250, number_of_buildings >= 10)


plotly::ggplotly(
  ggplot(test, aes(value, `Total_Total`, colour = number_of_buildings)) +
    geom_point(size = 3) +
